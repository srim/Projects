...
static int twolev_config[4] =
 ...
 static int tournament_config[4]=
 { /* selsize */4096, /* global_regsize */12, /* local_htb_size */1024, /* local_hrsize */10};
/* ######################################################################################################### */
...
/* l1 psedo associative cache config */
static char *cache_pseudo_dl1_opt;
...
/* branch predictor */
static struct bpred_t *pred, *global_pred, *local_pred, *T_selector_pred;
...
opt_reg_int_list(odb, "-bpred:tournament",
		   ...
		   "(<sel_size> <global_reg_size> <local_htb_size> <local_hrsize>)",
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(pred_type, "perfect"))
	...
	fatal("tournament pred config (<sel_size> <global_regsize> <local_htb_size> <local_hrsize>)");
      if (btb_nelt != 2)
	fatal("bad btb config (<num_sets> <associativity>)");
       			  
			  
		 local_pred = bpred_create(BPred2Level,
			  /* bimod table size */0,
			  /* 2lev l1 size */tournament_config[2],
			  /* 2lev l2 size */2^tournament_config[3],
			  /* meta table size */0,
			  /* history reg size */tournament_config[3],			  
			  /* history xor address */0,
			   /* btb sets */btb_config[0],
			  /* btb assoc */btb_config[1],
			  /* ret-addr stack size */ras_size);
		  ...
		  global_pred = bpred_create(BPred2Level,
			  /* bimod table size */0,
			  /* 2lev l1 size */1,
			  /* 2lev l2 size */2^tournament_config[1],
			  /* meta table size */0,
			  /* history reg size */tournament_config[1],			  
			  /* history xor address */0,
			   /* btb sets */btb_config[0],
			  /* btb assoc */btb_config[1],
			  /* ret-addr stack size */ras_size);
		  ...
		  T_selector_pred = bpred_create(BPred2bit,
			  /* bimod table size */tournament_config[0],
			  /* 2lev l1 size */0,
			  /* 2lev l2 size */twolev_config[0],
			  /* meta table size */0,
			  /* history reg size */0,			  
			  /* history xor address */0,
			   /* btb sets */btb_config[0],
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
  ...
  else /* dl1 is defined */
    {
		...
		if (sscanf(cache_pseudo_dl1_opt, "%s",
		  &pseudo) != 1)
				fatal("bad l1 Pseudo D-cache params: <true/false>");
				
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),pseudo,
      ...
      else
	{
	  ...
	  cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),FALSE,
  ...
  if (!mystricmp(itlb_opt, "none"))
	...
	fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>");
      itlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
			  ...
			  cache_char2policy(c), FALSE,itlb_access_fn,
			  /* hit latency */1);
    }
  ...
  if (!mystricmp(dtlb_opt, "none"))
	...
	fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>");
      dtlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
			  /* usize */sizeof(md_addr_t), assoc,
			  cache_char2policy(c),FALSE, dtlb_access_fn,
			  /* hit latency */1);
    }
...
ruu_fetch(void)
{
  ...
  for (i=0, branch_cnt=0;
	...
	if (MD_OP_FLAGS(op) & F_CTRL)
	{		
		if(!mystricmp(pred_type, "tournament"))	  
	    fetch_pred_PC =
	      tournament_bpred_lookup(T_selector_pred,global_pred, local_pred,
