...
/* MODIFIED TO IMPLEMENT PSEUDO COLUMN ASSOCIATIVE CACHE */
...
//Introduce a new macro to get a tag value with the high-order bit of the index appended at the end
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  if (!cp->sets[i].way_tail)
        ...
        //Make rehash bit as 1 for DL1 cache and a pseudo associative cache is used 
	if ((strcmp(cp->name,"dl1")==0) && (strcmp(cp->pseudo,"TRUE")==0))
	{
                for(k=0;k<nsets;k++)
                {
                      cp->sets[k].way_head->R=1;
					  cp->sets[k].way_tail->R=1;
                }
        }
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  //pseudo-associative DL1 cache code 
  if ((strcmp(cp->name,"dl1") == 0) && (strcmp(cp->pseudo,"TRUE") == 0))
  {
	struct cache_blk_t *blk1,*blk2,*blk3;
	md_addr_t new_set,tag_temp,tag_pseudo;
  ...
  if (cp->hsize)
	...
	tag_pseudo=	 CACHE_TAG_PSEUDOASSOC(cp, addr);
	
	
 	if (((blk->tag << 1)|(set >> (cp->tag_shift) - (cp->set_shift) -1)) == tag_pseudo && (blk->status & CACHE_BLK_VALID))//direct hit
                 goto cache_hit;//normal hit
        else if (!(blk->status & CACHE_BLK_VALID) && (blk->R == 1))//empty block with rehash bit set
        {
		 blk->R = 0;//indicates this is not a rehashed block
                 goto cache_miss;//normal miss
	}
	else if ((blk->status & CACHE_BLK_VALID) && (blk->R == 0) && (((blk->tag << 1)|(set >> (cp->tag_shift) - (cp->set_shift) -1)) != tag_pseudo)) //valid block with rehash bit cleared and not hit
	{
		 new_set=(set ^ (1<< (((cp->tag_shift)-(cp->set_shift))-1)));//get new set by swapping MSB of the original set
                 blk=cp->sets[new_set].way_head;//point blk to the new block
		 tag_temp=(blk->tag << 1)|(new_set >> ((cp->tag_shift) - (cp->set_shift)-1)) ;

		 if (((blk->tag << 1)|(set >> (cp->tag_shift) - (cp->set_shift) -1)) == tag_pseudo)//if hit in the swapped index position
			goto cache_hit_1;//normal hit followed by swapping the blocks
                 goto cache_miss_1;//normal miss followed by swapping the blocks
	}
	else if ((blk->status & CACHE_BLK_VALID) && (blk->R == 1) && (((blk->tag << 1)|(set >> (cp->tag_shift) - (cp->set_shift) -1)) != tag_pseudo))//valid block with rehash set and a miss
        {
                 blk->R = 0;//indicates this is not a rehashed block
                 goto cache_miss;//normal miss
	}
...
cache_miss_1:
  ...
  case LRU:
  case FIFO:
    repl = cp->sets[new_set].way_head;
    update_way_list(&cp->sets[new_set], repl, Head);
    break;
  case Random:
    {
      int bindex = myrand() & (cp->assoc - 1);
      repl = CACHE_BINDEX(cp, cp->sets[new_set].blks, bindex);
    }
    break;
  default:
    panic("bogus replacement policy");
  ...
  /* remove this block from the hash bucket chain, if hash exists */
  if (cp->hsize)
    unlink_htab_ent(cp, &cp->sets[new_set], repl);
  ...
  if (repl->status & CACHE_BLK_VALID)
    {
      ...
      if (repl_addr)
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, new_set);
      ...
      if (repl->status & CACHE_BLK_DIRTY)
	{
	  ...
	  cp->writebacks++;
	  lat += cp->blk_access_fn(Write,
				   CACHE_MK_BADDR(cp, repl->tag, new_set),
				   cp->bsize, repl, now+lat);
	}
    }
 // tag_temp=tag_pseudo;
  ...
  /* link this entry back into the hash table */
  if (cp->hsize)
    link_htab_ent(cp, &cp->sets[new_set], repl);

/*swapping the blocks after performing a normal miss on the new set*/

  blk1=cp->sets[set].way_head;
  blk2=cp->sets[new_set].way_head;
  blk3=blk1;
  blk1=blk2;
  blk2=blk3;
  blk3->R=blk2->R;
...
cache_hit_1:
  ...
  if (blk->way_prev && cp->policy == LRU)
    {
      /* move this block to head of the way (MRU) list */
      update_way_list(&cp->sets[new_set], blk, Head);
    }
...
/*swapping the blocks after performing a normal hit on the new set*/
...
else
{
 ...
 cache_hit_normal: /* slow hit handler */
