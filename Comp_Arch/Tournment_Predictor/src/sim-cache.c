...
"information is generated.\n"
  ...
  opt_reg_string(odb, "-pseudoassoc",
		 "Pseudo associativity implementation}",
		 &pseudo, "TRUE",
		 /* print */TRUE, NULL);
		  ...
		  int argc, char **argv)	/* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
	...
	// if (sscanf(cache_pseudo_dl1_opt, "%s", &pseudo) != 1)
	// fatal("bad psedo cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
  ...
  if (!mystricmp(itlb_opt, "none"))
	...
	//if (sscanf(cache_pseudo_dl1_opt, "%s", &pseudo) != 1)
      itlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
			  ...
			  cache_char2policy(c), pseudo, itlb_access_fn,
			  /* hit latency */1);
    }
  ...
  if (!mystricmp(dtlb_opt, "none"))
	...
	/*if (sscanf(cache_pseudo_dl1_opt, "%s", &pseudo) != 1)*/
      dtlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
			  /* usize */sizeof(md_addr_t), assoc,
			  cache_char2policy(c), pseudo, dtlb_access_fn,
			  /* hit latency */1);
    }
}
