...
/* cache access macros */
#define CACHE_TAG(cp, addr)	((addr) >> (cp)->tag_shift) /* srim: Changed tag shift here */
...
/* srim: Declare newset for Rehashed Location */
int newset;
newset= set ^ pow(2,((log_base2(nset))-1));
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  /* srim:  cp->tag_shift = cp->set_shift + log_base2(nsets)-1; */
  cp->tag_shift = cp->set_shift + log_base2(nsets)-1;
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if
	...
	{
		for (blk=cp->sets[newset].way_head; blk;  blk=blk->way_next)
		{
			if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
	    goto cache_hit;
	}
