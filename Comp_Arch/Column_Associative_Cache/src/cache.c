...
//introducing a new macro to get a tag value with the high-order bit of the index appended at the end
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (cp->hsize)
	...
	tag=(addr >> ((cp->tag_shift)-1));

	/*tag= (tag<<1)|(set >> ((cp->tag_shift)-(cp->set_shift)-1));
  	if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
		goto cache_hit;
	else if (blk->status & CACHE_BLK_VALID)//non-empty block
	{
		if (blk->R == 1)
		{
			blk->R=0;
		...
		{
			new_set=(set ^ (1<< (((cp->tag_shift)-(cp->set_shift))-1)));	
			blk=cp->sets[new_set].way_head;
			if (blk->tag == tag)
				goto cache_hit_1;
			else 
				goto cache_miss_1;
	...
	else
	{
 	...
 	if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))//direct hit
	...
	else if ((blk->status & CACHE_BLK_VALID) && (blk->R == 0) && (blk->tag != tag)) //valid block with rehash bit cleared and not hit
	{
		...
		new_set=(set ^ (1<< (((cp->tag_shift)-(cp->set_shift))-1)));//get new set by swapping MSB of the original set
		 ...
		 if (blk->tag == tag)//if hit in the swapped index position
			goto cache_hit_1;//normal hit followed by swapping the blocks
                 goto cache_miss_1;//normal miss followed by swapping the blocks
	}
	else if ((blk->status & CACHE_BLK_VALID) && (blk->R == 1) && (blk->tag != tag))//valid block with rehash set and a miss
        {
                 blk->R = 0;//indicates this is not a rehashed block
                 goto cache_miss;//normal miss
	}
