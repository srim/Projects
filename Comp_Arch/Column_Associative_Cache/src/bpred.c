...
{
  struct bpred_t *pred, *local_pred, *global_pred, *T_selector_pred;
  ...
  case BPredComb:
	...
	case tournament:
    /* use bimodal for tournament selector creation */
    pred->dirpred.bimod = 
      bpred_dir_create(BPred2bit, bimod_size, 0, 0, 0);
	local_pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, l1size, l2size, shift_width, xor);
	global_pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, l1size, l2size, shift_width, xor);
...
/* ##################################################################################################### */
 ...
 // BPRED LOCAL for local predictor
...
/* Local Bpred_Dir_Lookup Ends here
...
/* probe a predictor for a next fetch address, the predictor is probed
   ...
   (used for recovering ret-addr stack after mis-predict).  */
md_addr_t				/* predicted branch target addr */
tournament_bpred_lookup(struct bpred_t *T_selector_pred,	/* branch predictor instance */
		struct bpred_t *global_pred,	/* branch predictor instance */
		struct bpred_t *local_pred,	/* branch predictor instance */
	     md_addr_t baddr,		/* branch address */
	     md_addr_t btarget,		/* branch target if taken */
	     enum md_opcode op,		/* opcode of instruction */
	     int is_call,		/* non-zero if inst is fn call */
					 ...
					 * used on mispredict recovery */
{
 ...
 unsigned int selsize= selector-> dirpred.twolevel->config.two.l2size;	
 unsigned int sel_index=(baddr >> MD_BR_SHIFT) & ((1<< log_base2(selsize))-1);
 if(((T_selector)->confi.two.l2table[sel_index]>> 1 )& 1)
 	pred= global_pred;
  else
     pred= local_pred;  
 ...
 /* End of Tournament_Bpred_Lookup */
