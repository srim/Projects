...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_string(odb, "-cache:dl1",
		 "l1 data cache config, i.e., {<config>|none}",
		 &cache_dl1_opt, "dl1:128:32:4:0:0:l",
		 /* print */TRUE, NULL);

  opt_reg_note(odb,
"  The cache config parameter <config> has the following format:\n"
"\n"
"    <name>:<nsets>:<bsize>:<assoc>:<RRPV_Reg_Wid>:<PSEL_Reg_Wid>:<repl>\n"
"\n"
"    <name>         - name of the cache being defined\n"
"    <nsets>        - number of sets in the cache\n"
"    <bsize>        - block size of the cache\n"
"    <assoc>        - associativity of the cache\n"
"    <RRPV_Reg_Wid> - Width of the RRPV Register Width\n"
"    <PSEL_Reg_Wid> - Width of the PSEL Register\n"
"    <repl>         - block replacement strategy, 'l'-LRU, 'f'-FIFO, 'r'-random, 'h'-SRRIP_HP, 's'-SRRIP_FP, 'b'-BRRIP, 'd'-DRRIP,\n"
"\n"
...
"                -dtlb dtlb:128:4096:32:0:0:r\n"
  ...
  opt_reg_string(odb, "-cache:dl2",
		 "l2 data cache config, i.e., {<config>|none}",
		 &cache_dl2_opt, "ul2:1024:64:4:0:0:l",
		 /* print */TRUE, NULL);
  ...
  opt_reg_string(odb, "-cache:il1",
		 ...
		 &cache_il1_opt, "il1:512:32:1:0:0:l",
		 /* print */TRUE, NULL);
...
"      -cache:il1 il1:128:64:1:0:0:l -cache:il2 dl2\n"
"      -cache:dl1 dl1:256:32:1:0:0:l -cache:dl2 ul2:1024:64:2:0:0:l\n"
"\n"
"    Or, a fully unified cache hierarchy (il1 pointed at dl1):\n"
"      -cache:il1 dl1\n"
"      -cache:dl1 ul1:256:32:1:0:0:l -cache:dl2 ul2:1024:64:2:0:0:l\n"
	       );
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
  ...
  else /* dl1 is defined */
    {
      ...
      if (sscanf(cache_dl1_opt, "%[^:]:%d:%d:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc, &RRPV_M_Value, &PSEL_COUNTER_WIDTH, &c) != 7)
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<RRPV_Reg_Wid>:<PSEL_Reg_Wid>:<repl>");
	...
	{
	  if (sscanf(cache_dl2_opt, "%[^:]:%d:%d:%d:%d:%d:%c",
		     name, &nsets, &bsize, &assoc, &RRPV_M_Value, &PSEL_COUNTER_WIDTH, &c) != 7)
  ...
  if (!mystricmp(cache_il1_opt, "none"))
    ...
    {
      if (sscanf(cache_il1_opt, "%[^:]:%d:%d:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc, &RRPV_M_Value, &PSEL_COUNTER_WIDTH, &c) != 7)
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<RRPV_Reg_Wid>:<PSEL_Reg_Wid>:<repl>");
	...
	{
	  if (sscanf(cache_il2_opt, "%[^:]:%d:%d:%d:%d:%d:%c",
		     name, &nsets, &bsize, &assoc, &RRPV_M_Value, &PSEL_COUNTER_WIDTH, &c) != 7)
  ...
  if (!mystricmp(itlb_opt, "none"))
  ...
  else
    {
      //if (sscanf(itlb_opt, "%[^:]:%d:%d:%d:%c",
		// name, &nsets, &bsize, &assoc,&c) != 7)
		if (sscanf(itlb_opt, "%[^:]:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc, &c) != 5)
	fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>");
	//fatal("1110 bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<RRPV_Reg_Wid>:<PSEL_Reg_Wid>:<repl>");
  ...
  if (!mystricmp(dtlb_opt, "none"))
  ...
  else
    {
      if (sscanf(dtlb_opt, "%[^:]:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc,&c) != 5)
	fatal("1124 bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>");
...
#include "machine.def"
	      ...
	      && target_PC != pred_PC && br_pred_taken))
	{
	  /* Either 1) we're simulating perfect prediction and are in a
             ...
             perfect prediction, we've predicted the branch taken, but DRRIP Project
