...
unsigned int RRPV_M_Value=0;
unsigned int PSEL_COUNTER_WIDTH=0;
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  //#####DRRIP Project modification starts here
  cp->PSEL_COUNTER = 0;
  //#####DRRIP Project modification ends here
  ...
  for (bindex=0,i=0; i<nsets; i++)
    {
      ...
      if ( cp->policy == DRRIP) 
      {
            cp->sets[i].setType = getReplacementType(i, nsets);
      }
...
cache_char2policy(char c)		/* replacement policy as a char */
{
  ...
  case 'f': return FIFO;
  case 'h': return SRRIP_HP;
  case 's': return SRRIP_FP;
  case 'b': return BRRIP;
  case 'd': return DRRIP;
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  fprintf(stream,
	  ...
	  : cp->policy == FIFO ? "FIFO"
	  //DRRIP Project Modification starts here
	  : cp->policy == SRRIP_HP ? "Static RRIP Hit Priority"
	  : cp->policy == SRRIP_FP ? "Static RRIP Frequency Priority"
	  : cp->policy == BRRIP ? "Binomial RRIP"
	  : cp->policy == DRRIP ? "Dynamic RRIP"
	  //DRRIP Project Modification ends here
	  : (abort(), ""));
}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case DRRIP :
    {
        ...
        if ( cp->sets[set].setType == SRRIP_HP ) 
        {
            if ( cp->PSEL_COUNTER < ((1 << PSEL_COUNTER_WIDTH) - 1) ) 
            {
                cp->PSEL_COUNTER++;
            }
        }
        else if ( cp->sets[set].setType == BRRIP ) 
        {

            if ( cp->PSEL_COUNTER > 0 ) 
            {
                cp->PSEL_COUNTER --;
            }
            temppolicytype = BRRIP;
        }
        else 
        {
        // Using most significant bit of PSEL counter to select the RRIP type dynamically
            if ( (cp->PSEL_COUNTER >> (PSEL_COUNTER_WIDTH - 1)) == 1 ) 
            {
                temppolicytype = BRRIP;
            }
        }
  ...
  case BRRIP :
    {
       ...
       if(cp->policy == BRRIP)
       {
        ...
        while (!found_to_be_replaced)
        {
        
           // Search for block with RRPV value = 2^M-1 
            for (blk=cp->sets[set].way_head; blk; blk=blk->way_next)
            {
                if(blk->RRPV == ((1 << RRPV_M_Value) - 1))
                {
                    //printf("\n Block found to be replaced \n");
                    repl = blk;
                    found_to_be_replaced = 1;
                    break;
                }
            }
            // if no block is found with RRPV value = 2^M -1 then increment the value of RRPV
            if(!found_to_be_replaced)
            {
               // printf("\n No block found with max RRPV value");
                for (blk=cp->sets[set].way_head; blk; blk=blk->way_next)
                {
                    blk->RRPV ++;
                }
        ...
        if (repl)
        {
           // replaced block should have value 2^M-2
            repl->RRPV = (1 << RRPV_M_Value) - 2;
        }

        if(temppolicytype == BRRIP) 
        {
            int n = rand()% 10;
           
            if(n < 9)
            {
                repl->RRPV = (1 << RRPV_M_Value) - 1;
            }
            else
            { 
                repl->RRPV = (1 << RRPV_M_Value) - 2;
 ...
 cache_hit: /* slow hit handler */
  ...
  cp->hits++;
    ...
    else if ((cp->policy == SRRIP_HP) || (cp->policy == BRRIP))
    {
        blk->RRPV = 0;
    }
...
int getReplacementType( int set,int setcount )
{
    if ( set % (setcount/32) == 0 ) 
     {
           return SRRIP_HP;
     }
     else if ( (set+1) % (setcount/32) == 0 )
     {
         return BRRIP;
     }
     else 
